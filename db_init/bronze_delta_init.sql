...
USING DELTA LOCATION 's3://spark-read-study-bucket/pos_transaction_landing/raw_table'
TBLPROPERTIES (
...
CREATE OR REPLACE TABLE dbs_external_another.bronze.pos_transactions_quarantined(
  transaction_id String,
    ...
    terminal_id String,
    ingested_time TIMESTAMP,
    error_reason ARRAY<String>,
    is_corrupted BOOLEAN
)
USING DELTA LOCATION 's3://spark-read-study-bucket/pos_transaction_bronze/dlq_table'
TBLPROPERTIES (
  'delta.appendOnly' = 'true',
  'quality' ='bronze'
);
...
drop table dbs_external_another.bronze.pos_transactions_quarantined;
drop TABLE dbs_external_another.bronze.pos_transactions;
 SELECT * FROM dbs_external_another.bronze.pos_transactions_quarantined limit 1000;
 SELECT * FROM dbs_external_another.bronze.pos_transactions LIMIT 5000;